...
export default function AssistenteADWidget() {
  ...
  return (
    <>
      {/* CARD FLUTUANTE FIXO NO CANTO DIREITO (ASSISTENTE AD COM AS CORES FIÉIS DA LOGO) */}
      {isOpen && (
        <div className={`fixed bottom-24 right-6 z-50 w-96 bg-white rounded-3xl border border-[#E6EBF1] shadow-2xl overflow-hidden flex flex-col transition-all duration-300 ${
          ...
          {/* CABEÇALHO COM O GRADIENTE FIEL DA LOGO DA CHAMA */}
          <div className="bg-mesh-logo text-white p-5 space-y-3 relative overflow-hidden flex-shrink-0">
            ...
            {!isMinimized && (
              <div className="space-y-2 pt-1 relative z-10">
                ...
                <div className="mesh-3d-overlay text-white p-2.5 rounded-xl border border-white/30 shadow-md text-[11px] font-semibold text-center">
                  Fale comigo naturalmente. Exemplos: <span className="font-extrabold text-amber-300">“Exibir membros”</span> ou <span className="font-extrabold text-cyan-200">“Saldo Sicredi”</span>.
                </div>
              </div>
            )}
          </div>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSendPrompt(); }} className="relative flex items-center">
                  ...
                  <div className="absolute right-1.5 flex items-center gap-1">
                    <button 
                      ...
                      className="w-7 h-7 rounded-lg bg-flame-logo text-white flex items-center justify-center shadow hover:scale-105 transition-transform cursor-pointer"
                    >
      ...
      {/* BOTÃO FLUTUANTE NO CANTO INFERIOR DIREITO COM AS CORES DA CHAMA DA LOGO */}
      <div className="fixed bottom-6 right-6 z-50">
        <button
          onClick={() => { setIsOpen(!isOpen); setIsMinimized(false); }}
          className="relative group p-1 rounded-full bg-mesh-logo shadow-2xl hover:scale-110 transition-transform cursor-pointer flex items-center justify-center border-2 border-white/40"
          title="Abrir Assistente AD"
        >
          <span className="absolute -inset-1 rounded-full bg-flame-logo opacity-75 blur animate-pulse group-hover:opacity-100 transition duration-500" />
          ...
          <span className="absolute -top-2 -left-2 bg-flame-logo text-white text-[9px] font-extrabold px-2 py-0.5 rounded-full border border-white shadow">
            Assistente
          </span>
        </button>
      </div>
